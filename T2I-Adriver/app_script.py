...
def run(opt, IMAGE, PROMPT, CONDITIONINGS, CONDITIONING_WEIGHTS, name_ckpt="last.ckpt", diff_color_img=None, diff_depth_img=None, diff_seg_img=None, scale=7.5):
    ...
    with torch.inference_mode(), autocast('cuda'):
        ...
        if len(activated_conds) != 3:
            ...
            print("-> Load model ", f'T2I-Adriver/logs/coadapter-{activated_conds[0]}-{activated_conds[1]}-{activated_conds[2]}-big-shift-dev/checkpoints/{name_ckpt}')
            activated_conds.sort()
            global_opt.coadapter_ckpt = f'T2I-Adriver/logs/coadapter-{activated_conds[0]}-{activated_conds[1]}-{activated_conds[2]}-big-shift-dev/checkpoints/{name_ckpt}'
...
supported_cond = ["color", "depth", "seg"]
...
checkpoint_dir = f'T2I-Adriver/logs/coadapter-{supported_cond[0]}-{supported_cond[1]}-{supported_cond[2]}-big-shift-dev/checkpoints/'
