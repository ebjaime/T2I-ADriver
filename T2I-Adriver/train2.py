...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
    "--gpu", type=int, default=1, help="GPU index to use"
    )
...
class SetupCallback(Callback):
    ...
    def on_fit_start(self, trainer, pl_module):
        if trainer.global_rank == 0:
            ...
            OmegaConf.save(
                OmegaConf.create({"cli": self.cli_config}),
                os.path.join(self.cfgdir, "{}-cli.yaml".format(self.now)),
            )
...
class CUDACallback(Callback):
    ...
    def on_save_checkpoint(self, trainer, pl_module, checkpoint):
        ...
        try:
            ...
            rank_zero_info(
                f"Average checkpoint generating time: {epoch_time:.2f} seconds"
            )
...
class TimeBasedCheckpoint(ModelCheckpoint):
    def __init__(self, checkpoint_interval="00:15:00", *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.checkpoint_interval = self.parse_time(checkpoint_interval)
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        ...
        # Save checkpoint if the specified time interval has passed
        if current_time - self.last_checkpoint_time >= self.checkpoint_interval:
            self.last_checkpoint_time = current_time
            # Manually save checkpoint
            print("Saving checkpoint to " + self.dirpath + '/' + "checkpoint_" + datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S") + "_epoch-"+str(batch_idx)+ '.ckpt')
            trainer.save_checkpoint(self.dirpath + '/' + "checkpoint_" + datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S") + "_epoch-"+str(batch_idx)+ '.ckpt') 

    def on_train_epoch_end(self, trainer, pl_module):
        # This can be kept to ensure checkpointing at epoch end as well
        self.on_train_batch_end(trainer, pl_module, None, None, None, None)

        
def load_pretrained_ckp(model, ckpt, name):
    print(f"Load {name} state from {ckpt}")
    ...
    if name == "adapter":
        ...
        if len(new_state_dict) != 0:
            print(
                "Take care that you are using old version adapter as pretrained model"
            )
    ...
    if len(m) > 0:
        print(f"missing keys for {name}:")
        # print(m)
        # rank_zero_print(m)
    if len(u) > 0:
        print(f"unexpected keys for {name}:")
        # print(u)
        # rank_zero_print(u)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("-> Merge trainer cli with config")
        ...
        default_modelckpt_cfg = {
            "target": "T2I-Adriver.train2.TimeBasedCheckpoint",
            "params": {
                "dirpath": ckptdir,
                "filename": "{step:09}",
                "verbose": True,
        ...
        print("-> Add callback which sets up log directory")
        ...
        # imagenet_data = torchvision.datasets.FakeData(5, (3, 16,16), '.')
        ...
        print("-> Configure learning rate")
        bs, base_lr = config.data.params.batch_size, config.model.base_learning_rate
        if not cpu:
            ngpu = lightning_config.trainer.gpus # before len(lightning_config.trainer.gpus.strip(",").split(","))
        else:
            ngpu = 1
        ...
        if opt.train:
            try:
                ...
                # Save as checkpoint after training
                ckpt_path = os.path.join(ckptdir, "last.ckpt")
                trainer.save_checkpoint(ckpt_path)
