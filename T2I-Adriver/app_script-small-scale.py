...
def run(opt, IMAGE, PROMPT, CONDITIONINGS, CONDITIONING_WEIGHTS, name_ckpt="last.ckpt", diff_color_img=None, diff_depth_img=None, diff_seg_img=None, scale=7.5):
    ...
    with torch.inference_mode(), autocast('cuda'):
        ...
        if len(activated_conds) != 3:
            ...
            print("-> Load model ", f'T2I-Adriver/logs/coadapter-{activated_conds[0]}-{activated_conds[1]}-{activated_conds[2]}-no-prompts-new/checkpoints/{name_ckpt}')
            activated_conds.sort()
            global_opt.coadapter_ckpt = f'T2I-Adriver/logs/coadapter-{activated_conds[0]}-{activated_conds[1]}-{activated_conds[2]}-no-prompts-new/checkpoints/{name_ckpt}'
...
supported_cond = ["color", "seg", "sketch"]
...
# Input variables
train_dir = 'T2I-Adriver/dataset/val_6'
...
train_csv_path = 'T2I-Adriver/dataset/val_no_prompts.csv'
prompts_df = pd.read_csv(train_csv_path)
prompts_dict = dict(zip(prompts_df['Image'], prompts_df['Labels']))

# Create output directory
output_dir = 'T2I-Adriver/dataset/small_study_no_prompts'
...
def save_images(image_path, output_dir, conditionings, weights, name_ckpt="last.ckpt", diff_color_img_path=None, diff_depth_img_path=None, diff_seg_img_path=None, scale=7.5):
    ...
    for idx, img in enumerate(ims):
        ...
        if diff_color_img_path is not None:
            base_name = f'{conditionings[0]}_{conditionings[1]}_{conditionings[2]}_{name_ckpt}_{diff_color_img_path.split("/")[-1][:-4]}_{scale}_{image_name}.png'
        elif diff_depth_img_path is not None and diff_seg_img_path is not None:
            #base_name = f'{conditionings[0]}_{conditionings[1]}_{conditionings[2]}_{name_ckpt}_1.png'
            base_name = f'{conditionings[0]}_{conditionings[1]}_{conditionings[2]}_ground_truth_{name_ckpt}_{scale}_{image_name}.png' 
        else:
            base_name = f'generated_{image_name}_{name_ckpt}_{scale}.png'
    ...
    for idx, cond_name in enumerate(conditionings):
        ...
        cond_pil.save(os.path.join(sub_dir_path, f'{cond_name}_{image_name}_{name_ckpt}.png'))
...
checkpoint_dir = f'T2I-Adriver/logs/coadapter-{supported_cond[0]}-{supported_cond[1]}-{supported_cond[2]}-no-prompts-new/checkpoints/'
