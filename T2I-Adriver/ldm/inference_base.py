...
def get_sd_models_coadapter(opt):
    config = OmegaConf.load(f"{opt.config}")
    model = instantiate_from_config(config.model)
    load_pretrained_ckp(model, opt.sd_ckpt, "stable diffusion")
    if not opt.coadapter_ckpt == "":
        load_pretrained_ckp(model, opt.coadapter_ckpt, "coadapter")
...
def get_multiple_t2i_adapter_models(opt, adapter_finetunes_from, names):
    from train import load_pretrained_ckp
    config = OmegaConf.load(f"{opt.config}")
    model = load_model_from_config(config, opt.sd_ckpt, opt.vae_ckpt)
    for name, adapter_finetune_from in zip(names, adapter_finetunes_from):
        load_pretrained_ckp(model, adapter_finetune_from, name)
    model = model.to(opt.device)
...
def diffusion_inference(opt, model, sampler, adapter_features, append_to_context=None):
    ...
    if len(samples_latents.shape) == 4:
