...
parser.add_argument(
    "--config",
    type=str,
    default="configs/stable-diffusion/train_depth.yaml",
...
if __name__ == "__main__":
    ...
    experiments_root = osp.join("T2I-Adriver/experiments_depth", opt.name)
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        for _, data in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                z = model.module.encode_first_stage((data["im"] * 2 - 1.0).to(device))
                z = model.module.get_first_stage_encoding(z)

            optimizer.zero_grad()
            model.zero_grad()
            features_adapter = model_ad(data["im"].to(device)) #FIXME: before model_ad(data["depth"].to(device))
