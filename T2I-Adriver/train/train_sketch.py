...
if __name__ == "__main__":
    ...
    net_G = pidinet()
    ckp = torch.load("T2I-Adriver/models/table5_pidinet.pth", map_location="cpu")[
        "state_dict"
    ]
    ...
    experiments_root = osp.join("T2I-Adriver/experiments", opt.name)
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        print("-> Train data loader sampler", current_iter)
        # train
        ...
        if rank == 0:
            for data in val_dataloader:
                with torch.no_grad():
                    ...
                    print(
                        "-> Original sample saved in ",
                        os.path.join(
                            experiments_root, "visualization", "sample_%s.png" % data["sentence"]
                        ),
                    )
                    if not cv2.imwrite(
                        os.path.join(
                        ...
                        (tensor2img(data["im"]*255.0)).astype(np.uint8),
                    ):
                        raise Exception("Could not write sample image")
                    c = model.module.get_learned_conditioning(data["sentence"])
                    edge = net_G((data["im"]*255.0).cuda(non_blocking=True))[-1]
                    edge = edge > 0.5
                    edge = edge.float()
                    ...
                    if not cv2.imwrite(
                        ...
                        raise Exception("Could not write edge image")
                    features_adapter = model_ad(edge)
